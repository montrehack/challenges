...
session_start();
$info = (object)array();
$info->username = "login5";
$info->password = "S33OAgWlJ92j14p";
$info->database = "login5";
$info->level	= "login5";
$error = "";
...
function filter($string) {
	if(preg_match('/(union|char|and|or\s\S|\s\S|sleep|sha1|md5|if)/i', $string)) {
		return false;
	}
	return true;
}
...
<html>
	...
	<body>
		...
		<?php if(isset($_SESSION[$info->level])) {
			echo '<div class="flag">FLAG-58fti560fsu17tbkov61u0za9f</div>';
		} else { ?>	
		<section>
			<div class="title">
			Login portal 5			
